...
- **個股K線**：日/週/月線，疊加 MA5/20/60、布林通道(20,2)、KD，成交量副圖
- **籌碼面**：三大法人買賣超、大戶（10/20大股東）持股週變化
- **事件日曆**：除權息（自動）+ 法說會/總經/升降息（手動維護），含影響個股/族群分析
- **自動更新**：週一到週五 08:50 自動執行，頁面每60秒自動刷新

## 未來規劃

- **分點買賣超**：已評估過，台股唯一公開來源（證交所 bsr.twse.com.tw/bshtm）有驗證碼保護，
  需自建 OCR 模型或串接第三方解碼服務，屬於中大型且要長期維護的獨立工程（證交所改版驗證碼
  就得重做），列為未來獨立專案，本次視覺改版沒有一併處理。
